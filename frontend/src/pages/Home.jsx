...
const Home = () => {
  ...
  const fetchData = async (silent = false) => {
    ...
    try {
      ...
      if (!silent) console.warn("⚠️ Live API not reachable, falling back to mock data.", err.message);
      stData = mockStations;
  ...
  useEffect(() => {
    fetchData(); // Initial visible fetch
  ...
  }, [role]); // refresh data when role changes so we get latest live state
  ...
  if (role === 'admin') {
    mainContent = (
      <div className="flex-1 bg-slate-900 overflow-hidden relative flex flex-col">
        ...
        <AdminDashboard stations={stations} onRefresh={fetchData} />
      </div>
    );
  } else if (role === 'operator') {
    const assignedStation = stations.find(s => s._id === stationId);
    mainContent = (
      <div className="flex-1 bg-slate-900 overflow-hidden relative flex flex-col">
        <Navbar stations={stations} />
        <OperatorDashboard station={assignedStation} onRefresh={fetchData} />
      </div>
    );
  } else {
    // Regular User
